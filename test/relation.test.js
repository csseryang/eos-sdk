...
describe('Test Relation Contract: ', function () {
    ...
    it('Test relation', async function () {
        ...
        let [relation_a, user_a] = [relations[0], testers[0]];
        let [relation_b, user_b] = [relations[1], testers[1]];
        ...
        try {
            ...
            let inbox_content = await relation_a.get_inbox(user_b, null);
            console.log(outbox_content);
            console.log(inbox_content);

            expect(JSON.parse(outbox_content).result.sendmsgs).to.be.not.empty;
            expect(JSON.parse(inbox_content).result.receivemsgs).to.be.not.empty;

            console.log('send message successful');

            // Delete the last message
            let out_id = JSON.parse(outbox_content).result.msgid;
            let in_id = JSON.parse(inbox_content).result.msgid;

            await relation_a.delete_out_message(user_a, out_id, null);
            await relation_b.delete_in_message(user_b, in_id, null);

            outbox_content = await relation_a.get_outbox(user_a, null);
            inbox_content = await relation_b.get_inbox(user_b, null);
            console.log(outbox_content);
            console.log(inbox_content);
            ...
            expect(JSON.parse(outbox_content).result[0].sendmsgs).to.be.equal('[]');
            expect(JSON.parse(inbox_content).result[0].receivemsgs).to.be.equal('[]');
